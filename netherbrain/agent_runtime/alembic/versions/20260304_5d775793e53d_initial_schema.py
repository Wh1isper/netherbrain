...
Revision ID: 5d775793e53d
Revises:
Create Date: 2026-03-04 20:16:28.685871
"""
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX ix_conversations_title_trgm ON conversations USING gin (title gin_trgm_ops)")
    op.execute("CREATE INDEX ix_conversations_summary_trgm ON conversations USING gin (summary gin_trgm_ops)")
    op.create_table(
        "sessions",
    ...
    op.execute("CREATE INDEX ix_sessions_final_message_trgm ON sessions USING gin (final_message gin_trgm_ops)")
    op.create_table(
        "mailbox",
...
def downgrade() -> None:
    ...
    op.drop_index("ix_sessions_final_message_trgm", table_name="sessions")
    ...
    op.drop_index("ix_conversations_summary_trgm", table_name="conversations")
